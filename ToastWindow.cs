...
public sealed class ToastWindow:Form {
    ...
    public ToastWindow(string title,string message,int duration,Action open){
        ...
        ClientSize=new Size(390,140);BackColor=AppTheme.Background;ForeColor=AppTheme.Foreground;Font=new Font("맑은 고딕",10);AccessibleName="커밋 알리미 알림";
        var stripe=new Panel{BackColor=AppTheme.Primary,Bounds=new Rectangle(0,0,5,140),Anchor=AnchorStyles.Top|AnchorStyles.Bottom|AnchorStyles.Left};Controls.Add(stripe);
        ...
        var body=new Label{Text=message,Bounds=new Rectangle(64,47,302,48),ForeColor=AppTheme.Foreground,AutoEllipsis=true};Controls.Add(body);
        var hint=new Label{Text="클릭하여 열기",Bounds=new Rectangle(64,107,240,22),Font=new Font("맑은 고딕",9),ForeColor=AppTheme.Primary};Controls.Add(hint);
        var dismiss=new Button{Text="×",Bounds=new Rectangle(350,8,30,30),FlatStyle=FlatStyle.Flat,ForeColor=AppTheme.Primary,BackColor=BackColor,TabStop=false,AccessibleName="알림 닫기"};dismiss.FlatAppearance.BorderSize=0;Controls.Add(dismiss);dismiss.Click+=delegate{Close();};
