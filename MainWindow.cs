...
public class MainWindow : Form {
    ...
    public MainWindow(bool inTray) {
        ...
        FormClosing+=delegate(object sender,FormClosingEventArgs e){if(e.CloseReason==CloseReason.WindowsShutDown || e.CloseReason==CloseReason.TaskManagerClosing)exiting=true;if(!exiting){e.Cancel=true;Hide();ShowNotice("커밋 알리미","트레이에서 계속 실행합니다.\n종료는 트레이 아이콘을 우클릭하세요.");}else{timer.Stop();if(notification!=null&&!notification.IsDisposed)notification.Close();tray.Dispose();}};
    ...
    async Task Run(List<RepoEntry> entries,bool getZip){
        ...
        try{foreach(var e in entries){
            ...
            try{
                ...
                if(getZip){e.Status="ZIP 다운로드 중";var p=new Progress<long>(n=>{if(busy)state.Text=repo.Path+" · 다운로드 "+(n/1048576.0).ToString("0.0")+" MB";});saved=await Task.Run(()=>Backend.Download(repo,info.Sha,e.Folder,n=>((IProgress<long>)p).Report(n)));e.DownloadedPath=saved;e.DownloadedSha=info.Sha;e.ImportedZip=false;e.Status="ZIP 저장 완료";downloaded++;}
