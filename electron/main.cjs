...
let window,tray,backend,desktopNotices,origin,token,quitting=false,checkingQuit=false,windowReady=false,openRequested=false;
const smoke=process.argv.includes('--smoke-test');
const smokeData=process.argv.find(a=>a.startsWith('--smoke-data='))?.slice(13);
const startHidden=process.argv.includes('--tray')||process.argv.includes('--startup');
...
function openWindow(){openRequested=true;if(!windowReady||!window||window.isDestroyed())return;openRequested=false;if(window.isMinimized())window.restore();window.show();window.moveTop();window.focus();}
...
if(!app.requestSingleInstanceLock()){app.quit();}else{
 app.on('second-instance',(_event,args=[])=>{if(args.includes('--startup')){log('STARTUP_INSTANCE_IGNORED');return}log('SECOND_INSTANCE');openWindow();});
 ...
 app.whenReady().then(async()=>{
    ...
    backend=new Backend({
      ...
      startup:enabled=>{if(!smoke)app.setLoginItemSettings({name:'SimpleCommitWeb',openAtLogin:enabled,path:process.env.PORTABLE_EXECUTABLE_FILE||process.execPath,args:app.isPackaged?['--startup','--tray']:['"'+app.getAppPath()+'"','--startup','--tray']});}
    });
    ...
    window=new BrowserWindow({width:1200,height:800,minWidth:720,minHeight:560,show:false,backgroundColor:'#101513',title:'SimpleCommit',icon,autoHideMenuBar:true,webPreferences:{nodeIntegration:false,contextIsolation:true,sandbox:true,webSecurity:true,backgroundThrottling:false}});
    window.removeMenu();
    ...
    windowReady=true;
    if(openRequested||!startHidden)openWindow();
    if(smoke){
      if(startHidden&&window.isVisible())throw Error('Startup must remain in tray');
      log(startHidden?'STARTED_IN_TRAY':'STARTED_VISIBLE');
      ...
      if(process.argv.includes('--ui-test')){
        const checks=await window.webContents.executeJavaScript(`(async()=>{
          ...
          openRepo();await wait(240);expect(document.querySelector('#repo-dialog').open,'Dialog did not open');closeDialog(document.querySelector('#repo-dialog'));await wait(220);expect(!document.querySelector('#repo-dialog').open,'Dialog did not close');const toggle=document.querySelector('#notice-toggle');toggle.click();toggle.click();toggle.click();await wait(240);expect(!document.querySelector('#notice-panel').hidden&&!document.querySelector('#notice-panel').inert,'Interrupted fade did not reopen');document.querySelector('#notice-panel').click();await wait(220);expect(!document.querySelector('#notice-panel').hidden,'Inside click closed panel');document.body.click();await wait(220);expect(document.querySelector('#notice-panel').hidden,'Outside click did not close panel');await refresh();expect(document.querySelector('#rows').getAnimations({subtree:true}).length===0,'Polling animated rows');return {fades:true,version:state.version,rowHeight,autosave:true,notificationExpiry:true,persistentNotice:true,invalidInputProtected:true};
        })()`);
      ...
      const secondArgs=['--smoke-test','--smoke-data='+smokeData];
      const launcher=app.isPackaged?(process.env.PORTABLE_EXECUTABLE_FILE||process.execPath):process.execPath;
      const launchAgain=args=>{const child=spawn(launcher,app.isPackaged?args:[app.getAppPath(),...args],{windowsHide:true,stdio:'ignore'});child.on('error',error=>log('SECOND_ERROR '+error.message));};
      const waitEvent=name=>new Promise((resolve,reject)=>{const timer=setTimeout(()=>{app.removeListener('second-instance',handler);reject(Error(name+' timed out'))},20000);const handler=(_event,args)=>{if(args.includes('--startup')!==(name==='startup'))return;clearTimeout(timer);app.removeListener('second-instance',handler);resolve()};app.on('second-instance',handler)});
      window.hide();const startupEvent=waitEvent('startup');launchAgain([...secondArgs,'--startup','--tray']);await startupEvent;
      if(window.isVisible())throw Error('Repeated startup opened the window');log('STARTUP_TRAY_PASS');
      const manualEvent=waitEvent('manual');launchAgain(secondArgs);await manualEvent;
      if(!window.isVisible()||window.isMinimized())throw Error('Relaunch did not restore the existing window');log('RELAUNCH_RESTORE_PASS');
      window.minimize();openWindow();if(window.isMinimized()||!window.isVisible())throw Error('Minimized restore failed');
