...
public class WebHost {
    ...
    void RegisterStartup(){using(var key=Microsoft.Win32.Registry.CurrentUser.CreateSubKey(@"Software\Microsoft\Windows\CurrentVersion\Run")){if(config.StartWithWindows)key.SetValue("SimpleCommitWeb","\""+Application.ExecutablePath+"\" --tray");else key.DeleteValue("SimpleCommitWeb",false);}}
    ...
    [STAThread]public static void Main(string[] args){
        ...
        try{Application.EnableVisualStyles();var host=new WebHost(dir,port,import);host.Start();using(var stream=typeof(WebHost).Assembly.GetManifestResourceStream("app.ico"))using(var icon=new System.Drawing.Icon(stream))using(var tray=new NotifyIcon{Icon=icon,Text="SimpleCommit Web",Visible=true})using(var timer=new System.Windows.Forms.Timer{Interval=15000}){
            var menu=new ContextMenuStrip();menu.Items.Add("웹 앱 열기",null,delegate{host.Open();});menu.Items.Add("종료",null,delegate{lock(host.gate){if(host.busy){MessageBox.Show("진행 중인 작업이 끝난 뒤 종료해 주세요.");return;}host.stopping=true;host.listener.Stop();Application.Exit();}});tray.ContextMenuStrip=menu;tray.DoubleClick+=delegate{host.Open();};timer.Tick+=delegate{host.Tick();};timer.Start();host.Tick();if(!args.Contains("--tray"))host.Open();Application.Run();tray.Visible=false;}
